...
<xsl:value-of select="$className"/>(string name, string params);
    ...
    virtual mcsLOGICAL hasDefault_<xsl:value-of select="./name"/>();
    virtual mcsCOMPL_STAT getDefault_<xsl:value-of select="./name"/>(<xsl:call-template name="GetMcsTypeForParam"><xsl:with-param name="paramNode" select="."/></xsl:call-template> *<xsl:value-of select="./name"/>);</xsl:if>
