...
then
    ...
    then 
        ...
        sed -e "s#^OUTPUT_DIRECTORY.*#OUTPUT_DIRECTORY  = $OUTPUT#g;s#^INPUT .*#INPUT = $INPUT#;s#^HTML_HEADER.*#HTML_HEADER = $HEADER#;s/^PROJECT_NAME.*/PROJECT_NAME = \"$MODULE_NAME API\"/;s/^PROJECT_NUMBER.*/PROJECT_NUMBER = $REVISION/;s#^HTML_FOOTER.*#HTML_FOOTER = $FOOTER#;s#^FILE_PATTERNS.*#FILE_PATTERNS = $INPUT_FILES#;s#^GENERATE_MAN.*#GENERATE_MAN = YES#;s#^MAN_EXTENSION.*#MAN_EXTENSION = .$MAN_SECTION#;s#^MAN_LINKS.*#MAN_LINKS = YES#" $FILE | doxygen -
