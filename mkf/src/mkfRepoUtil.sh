...
function installMcsModule()
{
    module="$1"
    module_src_path="${module}/src"
    make -C "${module_src_path}" clean all ${MAN_CMD_DURING_INSTALL} install
}

function installMavenModule()
{
    module="$1"
    cd ${module}
    mvn install
...
function installModule()
{
    ...
    if [ -e "${module}/pom.xml" ]
    then
        installMavenModule ${module}
    else
        installMcsModule ${module}
    fi
    RET=$?
    if [ $RET -ne 0 ]
    then
...
function getProjectDesc()
{
    ...
    case "${project}" in
        ...
        ASPRO2 )
            echo ${JMMC_SVNROOT} MCS/${version}/{jmcs,deploy,jmal}
        ...
        AppLauncher )
            echo ${JMMC_SVNROOT} MCS/${version}/jmcs 
        ...
        MCS )
            echo ${JMMC_SVNROOT}
            echo MCS/${version}/{mkf,mcscfg,tat,ctoo,mcs,log,err,misc,thrd,timlog,fnd,misco,env,cmd,msg,sdb,evh,jmcs,deploy,jmal,modc,modcpp,modsh,jae,testgui} ;;
        OIFitsExplorer )
            echo ${JMMC_SVNROOT} MCS/${version}/{jmcs,deploy,jmal}
