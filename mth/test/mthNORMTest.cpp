 ...
 * JMMC project
 *
 * "@(#) $Id: mthNORMTest.cpp,v 1.1 2006-06-05 10:57:42 lsauge Exp $"
 *
 * History
 * -------
...
static char *rcsId __attribute__ ((unused)) = "@(#) $Id: mthNORMTest.cpp,v 1.1 2006-06-05 10:57:42 lsauge Exp $"; 
...
mcsDOUBLE
GetMean(mcsDOUBLE*vec,mcsUINT16 size)
{
    mcsDOUBLE sum=0.0;
...
void
PerformStat(STAT *stat)
{
    if(stat->size>1)
    {
        mcsDOUBLE mean = GetMean(stat->vec,stat->size);
        mcsDOUBLE s2  = 0.0;
        mcsDOUBLE s3  = 0.0;
        mcsDOUBLE s4  = 0.0;
...
mcsDOUBLE
signif(mcsDOUBLE x,mcsUINT16 digits)
{
...
int main(int argc, char *argv[])
{
    ...
    (void) PerformStat(&stat);
    
    printf("results (three digits) ------------\n");
    printf(" mean           : %7.3f\n",signif(stat.mean,2));
    printf(" standard dev   : %7.3f\n",signif(sqrt(stat.var),2));
    printf(" sknewness      : %7.3f\n",signif(stat.skweness,2));
    printf(" kurtosis       : %7.3f\n",signif(stat.kurtosis,2));
