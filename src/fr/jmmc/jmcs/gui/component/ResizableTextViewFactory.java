...
public class ResizableTextViewFactory {
     ...
     */
    private static void finishLayout(final JEditorPane editorPane, final JDialog dialog, final String text, final boolean modal) {
    ...
    public static void showUnsupportedJdkWarning() {
        ...
        if (jvmName != null && jvmName.toLowerCase().contains("openjdk")) {
            ...
            message += "Your Java Virtual Machine is an OpenJDK JVM, which has known bugs (SWING look and feel, fonts, PDF issues...) on several Linux distributions." + "<BR/><BR/>";
        }
        if (SystemUtils.IS_JAVA_1_5) {
            _logger.warn("Detected JDK 1.5 runtime environment.");
            shouldWarn = true;
            message += "<FONT COLOR='RED'>WARNING</FONT> : ";
            message += "Your Java Virtual Machine is of version 1.5, which has several limitations and is not maintained anymore security-wise." + "<BR/><BR/>";
        }
        if (shouldWarn) {
            message += "<BR/>" + "<B>JMMC strongly recommends</B> Sun Java Runtime Environments version 1.6 or newer, available at:" + "<BR/><BR/>" + "<CENTER><A HREF='http://java.sun.com/javase/downloads/'>http://java.sun.com/javase/downloads/</A></CENTER>" + "<BR/><BR/>" + "<I>Your current JVM Information :</I><BR/>" + "<TT>" + "java.vm.name = '" + jvmName + "'<BR/>" + "java.vm.vendor = '" + jvmVendor + "'<BR/>" + "java.vm.version = '" + jvmVersion + "'<BR/>" + "Java Home :<BR/>'" + jvmHome + "'" + "</TT>";
            message += "</BODY></HTML>";
